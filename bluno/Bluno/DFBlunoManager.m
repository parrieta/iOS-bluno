...
- (void)configureSensorTag:(CBPeripheral*)peripheral {
    ...
    blunoDev.bReadyToWrite = YES ;
    if (self.delegate) {
        [self.delegate readyToCommunicate:blunoDev] ;
    }
}
...
- (void)writeDataToDevice:(NSData*)data Device:(DFBlunoDevice*)dev {
    if (!_bSupported || data == nil || !dev.bReadyToWrite) {
        return ;
    }
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    if (self.delegate) {
        [self.delegate bleDidUpdateState:_bSupported] ;
    }
}
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    if (dev) {
        ...
        if (self.delegate) {
            DFBlunoDevice* blunoDev = [self.dicBlunoDevices objectForKey:key] ;
            [self.delegate didDiscoverDevice:blunoDev] ;
        }
    } else {
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    ...
    blunoDev.bReadyToWrite = NO ;
    if (self.delegate) {
        [self.delegate didDisconnectDevice:blunoDev] ;
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (self.delegate) {
        NSString* key = [peripheral.identifier UUIDString] ;
        DFBlunoDevice* blunoDev = [self.dicBlunoDevices objectForKey:key] ;
        [self.delegate didReceiveData:characteristic.value Device:blunoDev] ;
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (self.delegate) {
        NSString* key = [peripheral.identifier UUIDString] ;
        DFBlunoDevice* blunoDev = [self.dicBlunoDevices objectForKey:key] ;
        [self.delegate didWriteData:blunoDev] ;
    }
}
